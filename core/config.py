...
from .compat import PluginConfigBase
...
class PluginSectionConfig(PluginConfigBase):
    ...
    game_version: int = Field(
        default=25500,
        description="落雪 DX Rating 趋势接口的默认版本号（如 25500=舞萌DX 2026）",
    )
...
class ServerConfig(PluginConfigBase):
    ...
    enable_oauth: bool = Field(
        default=False,
        description="启用水鱼账号 OAuth（公开客户端，设备码绑定 + 每用户 refresh token）",
    )
    oauth_issuer: str = Field(
        default="https://auth.diving-fish.com",
        description="水鱼账号授权服务器根地址，端点从发现文档读取",
    )
    oauth_client_id: str = Field(
        default="",
        description="水鱼 OAuth 公开客户端 client_id（控制台登记，无 client_secret）",
    )
    oauth_scope: str = Field(
        default="prober.records.read",
        description="水鱼 OAuth scope，按实际需求保持最小化；上传需额外配置 prober.records.write",
    )
