...
采用模块化架构：命令层（mixin）、服务层、渲染层、客户端、存储分离，
本文件只负责组装与生命周期管理。
"""
...
from .stores import AliasStore, BindingStore, DivingFishBindingStore, LxnsBindingStore
...
class MaiMaiDXPlugin(
    ...
    async def on_load(self) -> None:
        ...
        self._df_oauth_bindings = DivingFishBindingStore(
            str(data_dir / "df_oauth_bindings.json")
        )
        await self._df_oauth_bindings.load()
        ...
        self._df_oauth: Optional[DivingFishOAuthService] = None
    ...
    async def on_unload(self) -> None:
        maidle = getattr(self, "_maidle", None)
        if maidle is not None:
            await maidle.stop()
        renderer = getattr(self, "_renderer", None)
        if renderer is not None:
            await renderer.close()

        if getattr(self, "_http_session", None):
            await self._http_session.close()
            self._http_session = None
        for attr in ("_df", "_lxns", "_lxns_auth", "_df_oauth", "_players", "_music", "_covers"):
    ...
    async def _ensure_clients(self) -> None:
        ...
        async with self._client_lock:
            ...
            self._df_oauth = DivingFishOAuthService(
                self.config.server,
                self._df,
                self._http_session,
                self._df_oauth_bindings,
            )
